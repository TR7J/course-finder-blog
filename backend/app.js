...
app.post('/post/:creatorId/follows', async (req, res) => {
    const creatorId = req.params.creatorId;
    console.log('Requested creatorId:', creatorId);
    const token = req.headers.authorization.split(' ')[1]; 
    try {
      // Verify the token and get the user's ID
      const decoded = jwt.verify(token, process.env.JWT_SECRET);
      ...
      // Find the post by ID
      const post = await Post.findById(creatorId);
  
      if (!post) {
        return res.status(404).json({ message: 'Post not found' });
      }
  
      // Check if the user already liked this pos
      const alreadyFollowed = post.creator.follows.includes(userId);
  
      if (alreadyFollowed) {
        // User already liked, so unlike it
        post.creator.follows.pull(userId); // Remove user from likes array
        post.creator.followCount = Math.max(0, post.likeCount - 1); // Decrement like count and ensure it's not less than 0
      } else {
        // User hasn't liked, so like it
        post.creator.follows.push(userId); // Add user to likes array
        post.creator.followCount += 1; // Increment like count
      }
  
      await post.save();
      res.status(200).json({ followCount: post.creator.followCount }); // Return updated like count

       // Fetch updated follow count for the specific user being followed
      const updatedPost = await Post.findById(creatorId).populate('creator', 'followCount');
      res.status(200).json({ followCount: updatedPost.creator.followCount }); // Return updated follow count
